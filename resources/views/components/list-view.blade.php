...
<div class="list-view bg-white/5 dark:bg-gray-900 rounded-lg p-4">
    <!-- Filter Bar -->
    <div class="mb-6 flex flex-col md:flex-row md:items-end md:space-x-4 space-y-2 md:space-y-0">
        <div class="flex-1">
            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Card Name</label>
...
@push('scripts')
<script src="{{ asset('build/list-view.js') }}"></script>
<script>
window.LIST_VIEW_CARDS = @json($board->lists->flatMap->cards->values());
</script>
@endpush
